...
list_Morris_iNOA = SplitObject(iNOA_Morris.integrated, 
                               split.by = "source")
list_adults = SplitObject(iNOA_Final_GSE124263_GSE112013_iNOA.integrated, 
                          split.by = "source")
...
a = grep("GSE124263",names(list_adults))
b = grep("GSE112013",names(list_adults))
list_CTL = list_adults[c(a,b)]

dat = 'Morris_CTL'
dir = paste('/Users/tascini.annasofia/OneDrive - Ospedale San Raffaele/prj_COSR/AlfanoM_904_infertilita_epigenetics/',dat,'/', sep ='')

somatic.list = c(morris_list, list_CTL)
#names(somatic.list) <- c('iNOA1', 'iNOA2','iNOA3', "Morris")

###### integration workflow #######
source('/Users/tascini.annasofia/OneDrive - Ospedale San Raffaele/prj_COSR/AlfanoM_904_infertilita_epigenetics/Testis_scRNA/integration_function.R')
...
vp_D <- VlnPlot(Seurat.object_s,
  ...
  scale_color_manual(values = c('orange', 'dodgerblue2'), labels = c(c1,c2))
...
ct = "LEY"; c1="iNOA"; c2="CTL"; gene = "CDKN1B"
Idents(Seurat.object) <- "celltype.cond"
azoospermia.response.gene <- FindMarkers(Seurat.object,
                                         ident.1 = paste(ct,"_",c1, sep=''),
                                         ident.2 = paste(ct,"_",c2, sep=''), 
...
somatic.list = c(morris_list, list_adults)
int_obj = integration_workflow(somatic.list = somatic.list, 
                               dataset_iNOA_dir = dir, 
                               dataset = dat)
...
png(paste(dir,"UMAP_cellAssigned_",dat,"_res",res,".png",sep=''),  
...
vp_D <- VlnPlot(Seurat.object_s,
  ...
  theme(plot.title = element_text(color="black", size=22, face="bold.italic"),
        ...
        panel.background = element_rect(fill = "white",colour = "black", size = 1, linetype = "solid")) +
  #geom_boxplot(width = 0.1, outlier.size=1) +
  scale_fill_manual(values = c('dodgerblue2', 'orange', 'red')) #, labels = c('iGCA','CTL'))
...
#### INSL3 #####
vp_INSL3 <- VlnPlot(Seurat.object_s,
                  feature = c('INSL3'),
                  #slot = "counts", 
                  #log = TRUE,
                  split.plot = F,
...
vp_INSL3
pdf(paste(dir, 'vpINSL3.pdf', sep=''), width = 6, height = 4)
print(vp_INSL3)
dev.off()
